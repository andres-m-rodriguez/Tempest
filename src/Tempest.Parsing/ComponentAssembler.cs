...
/// rules, and a diagnostic for every member that didn't.</summary>
...
/// <see cref="SourceEntries"/>s from any frontend) and produces what the tool uses
...
public static class ComponentAssembler
{
    public static AssembleResult Assemble(
        IEnumerable<SourceEntries> results,
        /// <summary>'\n'-joined @using blocks of every _Imports.razor, so razor-sourced
        /// type names (emitted as written) resolve in the generated files.</summary>
        IEnumerable<string>? importsUsings = null)
    {
        // A .cs method with [Event, Command] is found by both symbol providers; keep one copy.
        var methods = results.SelectMany(r => r.Methods)
            .GroupBy(m => (m.Namespace, m.ComponentName, m.MethodName, m.ParamType))
            .Select(g => g.First())
            .ToList();

        var reactives = results.SelectMany(r => r.Reactives)
